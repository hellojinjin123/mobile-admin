...
class CategoryModel extends Model {
	...
	private function _findUpLevels($id, $lang) {
		$pid = $this->getField('pid', array('my_id'=>$id,'lang'=>$lang));
		if( $pid != 0 ) {
	...
	//向下递归分类层次ID
	private function _findDownLevels( $pid, $lang  ) {
		$list = $this->where(array('pid'=>$pid,'lang'=>$lang))->select();
		foreach ($list as $key => $value) {
			$this->levelsDownStr .= $value['my_id'].',';
			$this->_findDownLevels($value['my_id'], $lang);
		}
