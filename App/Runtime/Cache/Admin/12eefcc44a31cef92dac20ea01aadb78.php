...
<input type="hidden" name="mid" value="<?php echo ($c_root); ?>" />
		...
		<tr>
			<th width="600">
				<div class="qw-fl grid-add-data">
					<input type="button" value="添加信息" onclick="goToUrl('__APP__/Admin/News/edit/mid/<?php echo ($c_root); ?>/cid/<?php echo ($cid); ?>');" class="button-img-add" />
				</div>
...
<table class="grid-table" border="1" cellpadding="0" cellspacing="0"> 
	...
	<?php if(!empty($dataList)): ?><tbody>
		<?php if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
		    ...
		    <td><a href="__APP__/Admin/News/edit/id/<?php echo ($vo["id"]); ?>/mid/<?php echo ($_GET['mid']); ?>/cid/<?php echo ($vo["category_id"]); ?>"><?php echo ($vo["title"]); ?></a>
		    ...
		    <!--  <td><a href="__APP__/Admin/News/comment/id/<?php echo ($vo["id"]); ?>" >评论</a></td>-->
		    <td>
		    <a href="__APP__/Admin/News/edit/id/<?php echo ($vo["id"]); ?>/mid/<?php echo ($_GET['mid']); ?>/cid/<?php echo ($vo["category_id"]); ?>"><img src="__ADMIN__/Public/imgs/edit.png" />编辑</a>&nbsp;&nbsp;
		    <a href="#" onclick="javascript:deleteData('__APP__/Admin/News/delete/mid/<?php echo ($_GET['mid']); ?>/id/<?php echo ($vo["id"]); ?>');"><img src="__ADMIN__/Public/imgs/cross.png" />删除</a></td>
		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
	</tbody>
	<?php else: ?>
	<tbody>
		<tr>
			<td colspan="11" align="center" style="color:red;">没有找到数据！！！<a href="__APP__/Admin/News/edit/cid/<?php echo $_GET["cid"];?>">添加</a></td>
		</tr>
	</tbody><?php endif; ?>
	<tfoot>
		<tr>
