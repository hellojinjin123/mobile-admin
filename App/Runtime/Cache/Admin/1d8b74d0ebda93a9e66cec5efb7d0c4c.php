...
<div class="nav-site"><?php getNavSite($c_root,$_GET['cid']);?> > 用户管理</div>
		...
		<tr>
			<th width="600">
				<div class="qw-fl grid-add-data">
					<input type="button" value="添加管理员" onclick="goToUrl('__APP__/Admin/User/edit/mid/<?php echo ($_GET['mid']); ?>');" class="button-img-add" />
				</div>
...
<table class="grid-table" border="1" cellpadding="0" cellspacing="0"> 
	...
	<tbody>
		<?php if(is_array($dataList)): $i = 0; $__LIST__ = $dataList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr> 
		    ...
		    <td> <?php if($vo['last_login_time']){ echo date('Y-m-d',$vo['last_login_time']); } ?> </td>
		    <td align="center"><?php echo ($vo["login_count"]); ?></td>
		    <td align="center">
		    <?php if($_SESSION['admin']['superadmin']==true){ ?>
		    <?php getCheckboxState($vo['id'],'is_publish',$vo['is_publish']);?>
		    <?php } ?>
		    </td>
		    <td>
		    ...
		    <?php if($_SESSION['admin']['superadmin']==true){ ?>
		    <a href="__APP__/Admin/User/edit/id/<?php echo ($vo["id"]); ?>">编辑</a>&nbsp;&nbsp;
		    <?php } ?>
		    <?php if($_SESSION['admin']['superadmin']==true && $vo['username']!='admin'){ ?>
		    <a href="__APP__/Admin/User/accredit/id/<?php echo ($vo["id"]); ?>">授权</a>&nbsp;&nbsp;
		    <a href="#" onclick="javascript:deleteData('__APP__/Admin/User/deleteById/mid/<?php echo ($_GET['mid']); ?>/id/<?php echo ($vo["id"]); ?>');"">删除</a>		    
		    <?php } ?>
